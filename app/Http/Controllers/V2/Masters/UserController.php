...
class UserController extends Controller
{
    function index(Request $request)
    {
        ...
        return view('v2.masters.user.index');
    }
    ...
    function password_Change_view(Request $request, $id)
    {
        ...
        if ($login_id == $user_id || $role == 'Admin' && $user_id != 1) {
            return view('v2.masters.user.password_reset', compact('user_id'));
        }
    ...
    public function create()
    {
        ...
        return view('v2.masters.user.add', compact(['roles', 'companys', 'bb_user']));
    }
    ...
    public function edit(Request $request)
    {
        ...
        $roles = Roles::get('name');
        $companys = CompanyMaster::get();
        return view('v2.masters.user.edit', compact(['roles', 'companys', 'user_name', 'user_email', 'selected_roles', 'selected_company', 'user_id']));
    }
    ...
    public function bin(Request $request)
    {
        ...
        if ($request->ajax()) {
            return dataTables::of($users)
                ...
                ->addColumn('action', function ($user) use ($login_id) {
                    $action = '';
                    $action = "<a href='restore/" . $user->id . "' class='btn btn-success btn-sm mr-2'><i class='fas fa-trash-restore'></i>Restore</a>";
                    return $action;
                })
        ...
        return view('v2.masters.user.bin');
    }

    public function restore($id)
    {
        $restore = User::withTrashed()->find($id)->restore();
        return redirect()->intended('/v2/master/users')->with('success', 'User  has been restored successfully');
    }
