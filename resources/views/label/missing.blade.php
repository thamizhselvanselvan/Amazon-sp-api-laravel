...
@section('content')
<form class="container-fluid" action="missing/order_id" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-2">
            <x-adminlte-select name="seller_id" label="Select Store">
                @foreach ($selected_store as $store)
                <option value="{{ $store->seller_id }},{{$store->country_code}}"> {{ $store->store_name }} [{{$store->country_code}}]</option>
                @endforeach
            </x-adminlte-select>
        </div>
    </div>
